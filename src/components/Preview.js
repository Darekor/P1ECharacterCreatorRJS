...
function Preview({attributes,race,pclass}) 
{
  ...
  return (
      <div style={{float:"right", backgroundColor:"#f1f1f1", padding:"25px", width:"30%"}}>
    <aside>
            ...
            <ul>
              {Object.entries(pclass.special[0]).map((val)=>(<li key={val[0]} title={pclass.classFeatures[val[1]].description}>{val[0]}</li>))}
            </ul>
    </aside>
    </div>
  )
